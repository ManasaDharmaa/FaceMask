...
@login_required  # Decorator to ensure only authenticated users can access the view
def voting_page(request):
    if request.user.is_authenticated and request.user.username == 'admin':  # Check if user is admin
        parties = Party.objects.all()
        # Fetch total count of votes
        total_votes = Vote.objects.count()
        return render(request, 'voting_page.html', {'parties': parties, 'total_votes': total_votes})
    else:
...
def admin_login(request):
    if request.method == 'POST':
        ...
        if user is not None and user.username == 'admin':
            login(request, user)
            return redirect('voting_page')  # Redirect to voting page after successful login
        else:
            messages.error(request, 'Invalid username or password')
    return render(request, 'admin_login.html')  # Render admin login page template
